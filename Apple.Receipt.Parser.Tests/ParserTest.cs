...
namespace Apple.Receipt.Parser.Tests
{
    public class ParserTest : IDisposable
    {
        ...
        public void TestParse()
        {
            ...
            var receipt = parserService.GetAppleReceiptFromBytes(data);
            if (receipt != null)
            {
                Assert.Equal("1", receipt.ApplicationVersion);
                Assert.Equal("1.0", receipt.OriginalApplicationVersion);
                Assert.Equal("1439452246000", receipt.ReceiptCreationDateMs);
                Assert.Equal(new DateTime(2015, 8, 13, 7, 50, 46, DateTimeKind.Utc), receipt.ReceiptCreationDateDt);
                ...
                Assert.Equal("com.mbaasy.ios.demo", receipt.BundleId);
            }
            else
            {
