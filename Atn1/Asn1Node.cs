	...
	public class Asn1Node : IAsn1Node
    {
        ...
        public static Asn1Node GetDecendantNodeByOid(string oid, Asn1Node startNode)
		{
			...
			for (int i = 0; i<startNode.ChildNodeCount; i++)
			{
				...
				int tmpTag = childNode.Tag & Asn1Tag.TagMask;
				if (tmpTag == Asn1Tag.ObjectIdentifier)
				{
        ...
        {
            int i;
            if (xNode.Tag == Asn1Tag.BitString)
            {
                subOffset++;
            }
            for (i=0; i<xNode.ChildNodeCount; i++)
            {
                ...
                tempNode._path = xNode._path + '/' + i.ToString();
        ...
        internal bool GeneralDecode(Stream xdata)
        {
            ...
            {
                if ((Tag & Asn1Tag.TagMask) <= 0 || (Tag & Asn1Tag.TagMask) > 0x1E)
                {
                    return false;
                }
            }
        ...
        internal bool ListDecode(Stream xdata)
        {
            ...
            try
            {
                ...
                using (Stream secData = new MemoryStream((int)_dataLength))
                {
                    ...
                    if (Tag == Asn1Tag.BitString) _dataLength++;
                    secData.Write(secByte, 0, secByte.Length);
                    secData.Position = 0;
                    while (secData.Position < secData.Length)
                    {
                        Asn1Node node = new Asn1Node(this, offset) { _parseEncapsulatedData = this._parseEncapsulatedData };
        ...
        internal bool InternalLoadData(Stream xdata)
        {
            ...
            int maskedTag = xtag & Asn1Tag.TagMask;
            if (((xtag & Asn1TagClasses.Constructed) != 0) 
                || (_parseEncapsulatedData 
				&& ((maskedTag == Asn1Tag.BitString)
                || (maskedTag == Asn1Tag.External)
                || (maskedTag == Asn1Tag.GeneralString)
                || (maskedTag == Asn1Tag.GeneralizedTime)
                || (maskedTag == Asn1Tag.GraphicString)
                || (maskedTag == Asn1Tag.Ia5String)
                || (maskedTag == Asn1Tag.OctetString)
                || (maskedTag == Asn1Tag.PrintableString)
                || (maskedTag == Asn1Tag.Sequence)
                || (maskedTag == Asn1Tag.Set)
                || (maskedTag == Asn1Tag.T61String)
                || (maskedTag == Asn1Tag.UniversalString)
                || (maskedTag == Asn1Tag.Utf8String)
                || (maskedTag == Asn1Tag.VideotextString)
                || (maskedTag == Asn1Tag.VisibleString)))
                )
            {
                if (ListDecode(xdata))
                {
